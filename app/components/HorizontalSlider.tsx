...
export default function HorizontalSlider({ slides }: HorizontalSliderProps) {
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (Math.abs(deltaX) > Math.abs(deltaY)) {
      ...
      // Vertical swipe
      if (deltaY > 50) {
        // Swipe up (next slide for mobile vertical scroll)
        if (currentSlide < slides.length - 1) {
          isScrollingRef.current = true;
          setCurrentSlide((prev) => prev + 1);
          setShowFooter(false);
  ...
  return (
    ...
    >
      ...
      {/* Slides Container - moves both horizontally and vertically */}
      <div
        className="flex h-screen transition-transform duration-700 ease-in-out"
        style={{
          width: `${slides.length * 100}vw`,
          transform: showFooter
            ? `translateX(-${currentSlide * 100}vw) translateY(-100vh)`
            : `translateX(-${currentSlide * 100}vw) translateY(0)`,
        }}
      >
        {slides.map((slide) => (
          <div
      ...
      {/* Navigation Dots */}
      <div
        className={`fixed bottom-8 left-1/2 z-10 flex -translate-x-1/2 gap-2 transition-opacity duration-700 ${
          showFooter ? "opacity-0" : "opacity-100"
        }`}
      >
        {slides.map((_, index) => (
